...
dma // {
  ...
  memprocfs = (dma.memprocfs.override { 
    python3 = prev.python312; 
  }).overrideAttrs (old: {
    # TODO python 3.12 temporarily
    nativeBuildInputs = (prev.lib.remove prev.python3 (old.nativeBuildInputs or [])) ++ [ prev.python312 ];
    buildInputs = (prev.lib.remove prev.python3 (old.buildInputs or [])) ++ [ prev.python312 ];

    # TODO GCC 15 temp fix
    NIX_CFLAGS_COMPILE = (old.NIX_CFLAGS_COMPILE or "") + " -Wno-error=implicit-function-declaration -Wno-error=incompatible-pointer-types";
